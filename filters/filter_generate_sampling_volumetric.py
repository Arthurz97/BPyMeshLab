...
class MESHLAB_PG_generate_sampling_volumetric(PropertyGroup, MeshLabFilterBase):
    ...
    def apply_filter(cls, context, props):
        ...
        if is_batch or len(original_objs) == 1:
            for obj in original_objs:
                ...
                if preserve and status == "FINISHED":
                    # O loop abaixo restaura as matrizes de TODAS as nuvens de pontos trazidas pela Classe Mestra
            ...
            msg_end = (
                "Batch Volumetric Sampling concluído"
                if len(original_objs) > 1
                else "Volumetric Sampling concluído"
            )
        ...
        # MODO GLOBAL (BOOLEAN MANIFOLD VIA COLLECTION)
            ...
            return status, "Global Volumetric Sampling gerado com sucesso."
