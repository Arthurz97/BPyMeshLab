...
class MESHLAB_PG_meshing_re_orient_faces_by_geometry(PropertyGroup, MeshLabFilterBase):
    ...
    def apply_filter(cls, context, props):
        ...
        # Processamento iterativo (Batch) é a única rota segura imposta
        for obj in original_objs:
        ...
        msg_end = (
            "Batch Reorient Faces concluído"
            if len(original_objs) > 1
            else "Reorient Faces concluído"
        )
