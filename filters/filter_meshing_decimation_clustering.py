...
class MESHLAB_PG_meshing_decimation_clustering(PropertyGroup, MeshLabFilterBase):
    ...
    def apply_filter(cls, context, props):
        ...
        if is_batch or len(original_objs) == 1:
            ...
            msg_end = (
                "Batch Clustering Decimation concluído"
                if len(original_objs) > 1
                else "Clustering Decimation concluído"
            )
        ...
        # MODO GLOBAL (BOOLEAN MANIFOLD VIA COLLECTION)
            ...
            # Se a união falhar (ex: geometria não-manifold gerando objeto vazio)
            ...
            return status, "Global Clustering Decimation gerado com sucesso."
