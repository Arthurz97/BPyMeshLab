...
class MESHLAB_PG_apply_coord_laplacian_smoothing_scale_dependent(
    PropertyGroup, MeshLabFilterBase
):
    pymeshlab_filter = "apply_coord_laplacian_smoothing_scale_dependent"
    requires_selection = True
    ignore_selection_count = True
    shade_flat = True
    remove_attributes = ["custom_normal", "sharp_edge", "sharp_face"]

    # Tratamento do delta como medida absoluta no mundo convertida internamente para C++
    ...
    def apply_filter(cls, context, props):
        ...
        msg_end = (
            "Batch ScaleDependent Laplacian Smooth concluído"
            if len(original_objs) > 1
            else "ScaleDependent Laplacian Smooth concluído"
        )
    ...
    delta: FloatProperty(
        name="delta (abs and %)",
        description="The diffusion time step (delta) bounding the displacement per iteration.",
        subtype="DISTANCE",
        unit="LENGTH",
        default=0.01,
        min=0.0,
