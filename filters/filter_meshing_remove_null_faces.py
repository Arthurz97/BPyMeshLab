...
class MESHLAB_PG_meshing_remove_null_faces(PropertyGroup, MeshLabFilterBase):
    pymeshlab_filter = "meshing_remove_null_faces"
    ...
    def apply_filter(cls, context, props):
        ...
        msg_end = (
            "Batch Remove Zero Area Faces concluído"
            if len(original_objs) > 1
            else "Remove Zero Area Faces concluído"
        )
