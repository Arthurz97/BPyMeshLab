...
class MESHLAB_PG_generate_sampling_element(PropertyGroup, MeshLabFilterBase):
    ...
    def apply_filter(cls, context, props):
        ...
        if is_batch or len(original_objs) == 1:
            ...
            msg_end = (
                "Batch Mesh Element Sampling concluído"
                if len(original_objs) > 1
                else "Mesh Element Sampling concluído"
            )
        ...
        # MODO GLOBAL (BOOLEAN MANIFOLD VIA COLLECTION)
            ...
            return status, "Global Mesh Element Sampling gerado com sucesso."
