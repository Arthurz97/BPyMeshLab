...
class MESHLAB_PG_meshing_cut_along_crease_edges(PropertyGroup, MeshLabFilterBase):
    ...
    def apply_filter(cls, context, props):
        ...
        # Trava de segurança: Previne o erro "Current mesh does not have Any Faces"
        valid_objs = [obj for obj in original_objs if len(obj.data.polygons) > 0]
        if not valid_objs:
            return (
                "CANCELLED",
                "A malha selecionada não possui faces (polígonos) para ser cortada.",
            )
        ...
        msg_end = (
            "Batch Cut along crease edges concluído"
            if len(valid_objs) > 1
            else "Cut along crease edges concluído"
        )
        return overall_status, f"{msg_end} em {len(valid_objs)} objeto(s)."
